...
class GuiRegressionTests(unittest.TestCase):
    ...
    def test_batch_scan_replaces_recognition_mechanism_with_api_exam_source(self) -> None:
        ...
        self.assertIn('def _numeric_widths_from_subject_cfg(exam_code_text: str) -> dict[int, int]:', source)
        ...
        self.assertIn('numeric_widths = _numeric_widths_from_subject_cfg(result.exam_code)', source)
        self.assertIn('row_numeric_layout = [(q_no, numeric_widths.get(int(q_no), int(width))) for q_no, width in row_numeric_layout]', source)
