...
class MainWindow(QMainWindow):
    ...
    def _on_batch_subject_changed(self, _index: int) -> None:
        prev_subject = str(self.active_batch_subject_key or "").strip()
        if prev_subject:
            self._cache_working_batch_state(prev_subject)
        ...
        if self.active_batch_subject_key and self._restore_cached_working_batch_state(self.active_batch_subject_key):
            return
    ...
    def run_batch_scan(self) -> None:
        ...
        if subject_cfg:
            ...
            if answer_key_key and self._subject_imported_answer_keys_for_main(subject_cfg):
                repo = AnswerKeyRepository()
                for exam_code, kd in self._subject_imported_answer_keys_for_main(subject_cfg).items():
                ...
                self.imported_exam_codes = sorted(
                    str(k) for k in self._subject_imported_answer_keys_for_main(subject_cfg).keys()
                )
